...
// handleGetAgentLocations returns all agents with known locations for the map.
func handleGetAgentLocations(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		agents, err := db.GetAgentLocations(r.Context())
		if err != nil {
...
func handleUpdateAgentLocation(db *store.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var body struct {
			AgentID  string  `json:"agent_id"`
			...
			Source   string  `json:"source"`    // gps | wifi | ip
